...
function main() {
  ...
  function eventCharCode(opNum) {
    opNum.onkeypress = function (event) {
      // Permite solo números del 0 al 9 y un signo de menos al principio
      if (event.charCode >= 48 && event.charCode <= 57 || event.charCode === 45 && !opNum.value.includes('-')) {
  ...
  function focus(opUno, opDos) {
    opUno.onchange = function () {
      opDos.focus();
    };
    opDos.onchange = function () {
      opUno.focus();
    };
  }
  function controllWheel(opNum) {
    opNum.onmousewheel = function (event) {
      event.preventDefault();
    };
  }
